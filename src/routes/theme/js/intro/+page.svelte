<script lang="ts">
    ...
    import themeSource from '$lib/assets/theme/js/intro.yaml?raw';
    const defaultTheme: any = [
        {
            ...
            description: 'Qué es JavaScript, dónde corre y cómo se usa para construir aplicaciones web modernas.',
            coverImg: 'intro'
        },
        {
            template: 'card',
            ...
            description: 'Lenguaje de programación estándar (ECMAScript) que se ejecuta en navegadores y servidores (Node.js).'
        }
    ];
    let theme: any = defaultTheme;
    onMount(() => {
        try {
            theme = YAML.parse(themeSource);
            currentTheme = theme[themeId] || {};
            themeSlug = (theme[0] && theme[0].slug) || 'intro'
            themeNumb = (theme[0] && theme[0].theme) || 0
            themeColor = (theme[0] && theme[0].color) || 'yellow'
        } catch (e) {
        }
    });
    ...
    let themeSlug = (theme[0] && theme[0].slug) || 'intro'
    let themeNumb = (theme[0] && theme[0].theme) || 0
    let themeColor = (theme[0] && theme[0].color) || 'yellow'
</script>
